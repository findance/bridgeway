...
/// @notice Preflight deterministic BGW/BGW-GOV token addresses before broadcast.
///
/// Required env vars:
...
contract PredictDeterministicTokens is Script {
    function run() external view {
        ...
        console.log("BGW_TOKEN_PREDICTED=", predictedBGW);
        console.log("BGW_TOKEN_CODE_LENGTH=", predictedBGW.code.length);
        console.log("GOV_TOKEN_PREDICTED=", predictedGov);
        console.log("GOV_TOKEN_CODE_LENGTH=", predictedGov.code.length);
        console.log("BGW_TOKEN_SALT:");
        console.logBytes32(BridgewayDeterministicDeploy.bgwTokenSalt());
        console.log("GOV_TOKEN_SALT:");
